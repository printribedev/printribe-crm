...
const GREEN = "#1A7A4A", ORANGE = "#E67E22";
...
function Modal({ client, onSave, onClose, onDelete, deleteError }: {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }} onClick={onClose}>
      <div style={{ background: WHITE, borderRadius: 14, width: "100%", maxWidth: 560, maxHeight: "90vh", overflowY: "auto", padding: 28 }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ marginTop: 16, padding: "12px 14px", background: BG, borderRadius: 8 }}>
  ...
  return (
    <div style={{ padding: "26px 28px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 22 }}>
        <div>
          <div style={{ fontSize: 20, fontWeight: 700, color: BLACK }}>Clients</div>
          <div style={{ fontSize: 12, color: MID, marginTop: 3 }}>{clients.length} clients · Click a card to expand details</div>
        </div>
        <button onClick={() => { setDeleteError(""); setModal({}); }} style={{ fontSize: 12, fontWeight: 600, padding: "8px 16px", borderRadius: 7, background: R, color: WHITE, border: "none", cursor: "pointer" }}>
          + Add client
        </button>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 12 }}>
        {sorted.map(c => (
          <div key={c.id} style={{ background: WHITE, border: `1px solid ${selected === c.id ? R : BORDER}`, borderRadius: 10, padding: "18px 20px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
              ...
              <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                ...
                <button onClick={() => { setDeleteError(""); setModal(c); }} style={{ fontSize: 11, padding: "4px 10px", borderRadius: 6, border: `1px solid ${BORDER}`, background: WHITE, cursor: "pointer", color: MID }}>
                  Edit
                </button>
              </div>
            </div>
